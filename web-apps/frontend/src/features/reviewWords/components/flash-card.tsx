...
export default function FlashCard({ word, onDelete, isPending }: FlashCardProps) {
  ...
  const getCategoryColor = (category: string | null) => {
    ...
    return colors[category as keyof typeof colors] || "border-gray-400 bg-gray-50 dark:bg-gray-950/30";
  };
        ...
        >
          <div className="h-full flex flex-col p-4 rounded-xl border-2 shadow-lg overflow-hidden">
            <div className="flex-1 overflow-y-auto space-y-3 mb-4">
              <div>
                <h3 className="font-semibold text-base mb-1 text-foreground">意味</h3>
                <p className="text-sm leading-relaxed text-foreground break-words">{word.meaning}</p>
              </div>
              
              <div>
                <h3 className="font-semibold text-base mb-1 text-foreground">翻訳</h3>
                <p className="text-sm text-foreground">{word.translation}</p>
              </div>

              {word.example && (
                <div>
                  <h3 className="font-semibold text-base mb-1 text-foreground">例文</h3>
                  <p className="text-sm italic text-muted-foreground leading-relaxed break-words">{word.example}</p>
                </div>
              )}

              {word.partOfSpeech && (
                <div>
                  <h3 className="font-semibold text-sm mb-1 text-foreground">品詞</h3>
            ...
            <div className="flex gap-2 mt-auto flex-shrink-0" onClick={(e) => e.stopPropagation()}>
              <Button
                variant="destructive"
                size="sm"
                onClick={(e) => {
              ...
              <Button variant="outline" size="sm" className="flex-1" asChild>
                <Link href={`/update/${word.id}`} onClick={(e) => e.stopPropagation()}>
                  更新
                </Link>
              </Button>
            </div>
